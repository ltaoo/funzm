...
class Helper<T> {
  ...
  // 响应处理器
  private fakeProcessor: OriginalResponseProcessor = RESPONSE_PROCESSOR;
  ...
  constructor(fetch: any, options: InitialOptions = {}) {
    ...
    if (processor) {
      this.fakeProcessor = (originalResponse) => {
        const nextResponse = {
          ...this.response,
  ...
  private initialize = (options: InitialOptions) => {
  ...
  fetch = async (...args: any[]) => {
    ...
    try {
      ...
      let response = this.fakeProcessor<T>(originalResponse);
  ...
  /**
   * 根据最后一条记录加载更多（当列表存在删除操作时需要使用该方法）
   */
  loadMoreWithLastItem = () => {
    if (this.response.loading || this.response.noMore) {
      return;
    }
    this.params.page += 1;
    this.params.start =
      this.response.dataSource[this.response.dataSource.length - 1][
        this.rowKey
      ];
    return this.fetch(this.params, { concat: true });
  };
  ...
  deleteItems = async <T = any>(items: T[]) => {
    ...
    const nextResponse = this.fakeProcessor(originalResponse);
    this.response = {
      ...this.response,
      ...nextResponse,
