...
export const forgot: Handler = async (req) => {
  ...
  const ambiguous =
    "A link to reset your password will be sent to your email address if an account exists";

  // Check for existing user email
  const userid = await Email.findEmailService(input.email);
...
export const reset: Handler = async (req) => {
  ...
  let user = await User.findUserService(userid);
